...
export function PageNumber(props){
    ...
    // const numberOfEntry = props.totalEntry;
    // console.log(numberOfEntry);
    // if(Number(numberOfEntry) > 0){
    //     let num = Number(numberOfEntry)/5;
    //     setNumberOfPage(num)
    // }
    ...
    function selectedPage(pageNumber){
        props.setSkip(pageNumber - 1)
        setSelectPage(pageNumber - 1)
    }
    function handleRight(){
        if(selectPage === props.pages-1){
            return
        }
        props.setSkip(selectPage + 1)
        ...
        <div className="text-center">
            <div className="inline-flex rounded-xl border border-[#e4e4e4] bg-white p-4">
                ...
                {pageNumbers && pageNumbers.map((pageNumber) => (<li className="px-[6px]">
                    ...
                    key={pageNumber}
                    className="hover:bg-primary hover:border-primary focus:bg-[#000] focus:text-white flex h-9 w-9 items-center justify-center rounded-md border border-[#EDEFF1] text-base text-[#838995] hover:text-white"
                    >
                    {pageNumber}
                    </a>
                </li>))}
